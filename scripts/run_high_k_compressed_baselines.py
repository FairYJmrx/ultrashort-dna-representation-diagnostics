...
from scripts.run_stage2_representation_grid import balanced_sample, paired_subsets, parse_csv_list, parse_int_list, set_global_seed  # noqa: E402
...
def fast_minhash_matrix(sequences: list[str], k: int, sketch_size: int, canonical: bool = True, seed: int = 0) -> np.ndarray:
    """Fast deterministic MinHash-style sketch using universal hashing over token hashes."""
    ...
    for seq in sequences:
        ...
        vals = ((a[:, None] * h[None, :] + b[:, None]) % prime).min(axis=1).astype(np.float64)
        rows.append(vals / float(prime))
...
def build_named_features(sequences: list[str], length: int, train_indices: list[int], representation: str, seed: int, high_k: int, target_dim: int) -> tuple[np.ndarray, int, str]:
    ...
    if representation == "minhash_k15_s222":
        x = fast_minhash_matrix(sequences, k=high_k, sketch_size=target_dim, canonical=True, seed=seed)
        return np.asarray(x, dtype=np.float64), int(target_dim), REP_LABELS[representation]
                    ...
                    metrics.update(
                        {
                            ...
                            "high_k": int(high_k) if rep in {"hash_k15_d222", "rp_ck15_d222", "minhash_k15_s222"} else np.nan,
                            "target_dim": int(target_dim) if rep in {"hash_k15_d222", "rp_ck15_d222", "minhash_k15_s222"} else np.nan,
                        }
                    )
                    rows.append(metrics)
...
def summarize(stability: pd.DataFrame, readout: pd.DataFrame, out_dir: Path) -> None:
    lines = [
        ...
        "",
        "This audit compares CK4P-MSP with dimension-matched high-specificity k-mer compression baselines. The high-k controls use canonical k=15 signals compressed to approximately the CK4P-MSP feature budget through MinHash-style signatures, hashing trick counts or sparse random projection.",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--representations", default="ckmer4_count_l2,ckmer5_count_l2,ckmer4_property_l2,ckmer4_property_multiscale_mean_l2,minhash_k15_s222,hash_k15_d222,rp_ck15_d222")
