...
FIGURES = {
    'Figure 1': ('nature_fig1_framework.png', 'Representation-diagnostic framework. Controlled clean/perturbed read pairs are encoded into composition, global biochemical and coarse positional-property blocks, then evaluated along stability, retrieval, readability and compactness axes.'),
    'Figure 2': ('nature_fig2_compact_stability.png', 'Seven-group K/P/MSP contribution audit. Mean standardized drift, grouped local-change macro-F1 and nearest-clean retrieval are shown for all non-empty block combinations; the complete representation is a balanced trade-off rather than the winner on every individual axis.'),
    'Figure 3': ('nature_fig3_ck4p_msp_tradeoff.png', 'Dimension-matched compact stability audit. CK4P-MSP is compared with composition, reduction and compressed high-k vector baselines under the same perturbation grid; MinHash is evaluated separately in its native Jaccard geometry.'),
    'Figure 4': ('nature_fig4_external_probes.png', 'External probes separate simulator-derived stability from task-limited readability. ART supports paired stability consistency, CAMI_TOY_low supplies a coarse target/background readout, and CAMI II marine supplies an anonymous-read stability probe without taxonomic labels.'),
    'Figure 5': ('nature_fig5_full_position_upper_bound.png', 'Full-position matrices define a positional-readability upper bound at substantially higher feature dimension. CK4P-MSP retains only a compact coarse positional summary.'),
    'Figure 6': ('nature_fig6_local_mutation_sensitivity.png', 'Local-change readout and distance-ratio boundary. Full-position property encodings provide stronger distance amplification, whereas CK4P-MSP preserves high grouped local-change readout at compact dimensionality.'),
    'Supplementary Figure S1': ('supp_fig_s1_baseline_audit.jpg', 'Baseline and mixed-metric audit.'),
    'Supplementary Figure S2': ('supp_fig_s2_mi_audit.png', 'Estimator-dependent empirical MI proxy audit.'),
    ...
    'Supplementary Figure S5': ('supp_fig_s5_p_channel_counterfactual_audit.jpg', 'P-channel sequence-linkage counterfactual audit.'),
    'Supplementary Figure S6': ('supp_fig_s6_msp_bin_gamma_sensitivity.png', 'MSP binset and gamma-sensitivity audit.'),
    'Supplementary Figure S7': ('supp_fig_s7_method_hardening_audit.png', 'Dimension-matched high-k vector and native MinHash audit.'),
    'Supplementary Figure S8': ('supp_fig_s8_redundancy_runtime_audit.png', 'P/MSP relation and runtime audit.'),
    'Supplementary Figure S9': ('supp_fig_s9_p_msp_relation_audit.jpg', 'P/MSP correlation, canonical-correlation and row-alignment audit.'),
...
TABLES = {
    ...
    'Table 4': ('nature_table4_local_mutation_sensitivity.csv', 'Seven-group block-ablation metrics.'),
...
def strip_working_notes(text):
    ...
    for line in text.splitlines():
        ...
        if heading.startswith('Figure and Table Placement Summary'):
            skip = True
        if not skip:
            lines.append(line)
    return '\n'.join(lines).strip()
...
def clean_inline(text):
    def latex_to_text(match):
        ...
        replacements = [
            ...
            (r'\mathbf{K}_i', 'Kᵢ'), (r'\mathbf{K}_j', 'Kⱼ'),
            (r'\mathbf{P}_i', 'Pᵢ'), (r'\mathbf{P}_j', 'Pⱼ'),
            (r'\mathbf{M}_i', 'Mᵢ'), (r'\mathbf{M}_j', 'Mⱼ'),
            (r'\mathbf{x}_i', 'xᵢ'), (r'\mathbf{x}_j', 'xⱼ'),
            (r'\mathrm{mix}', 'mix'), (r'\alpha', 'α'), (r'\beta', 'β'),
            (r'\gamma', 'γ'), (r'\times', '×'), (r'\in', '∈'),
            (r'\leq', '≤'), (r'\geq', '≥'), (r'\mid', '|'),
            (r'\left\|', '‖'), (r'\right\|', '‖'), (r'\|', '‖'),
            (r'\left', ''), (r'\right', ''), (r'\,', ' '), (r'\;', ' '),
            ('_2', '₂'), ('^2', '²'), ('_i', 'ᵢ'), ('_j', 'ⱼ'),
    ...
    text = re.sub(r'\$([^$]+)\$', latex_to_text, text)
    text = text.replace('`', '')
    text = text.replace('–', '-')
    text = text.replace('—', '-')
    text = text.replace('‑', '-')
...
def add_equation_block(doc, eq_text):
    ...
    if not compact:
        return
    if all(token in compact for token in (r'\hat{\mathbf{K}}_i=', r'\hat{\mathbf{P}}_i=', r'\hat{\mathbf{M}}_i=')):
        rendered = 'K̂ᵢ = Kᵢ/‖Kᵢ‖₂,   P̂ᵢ = Pᵢ/‖Pᵢ‖₂,   M̂ᵢ = Mᵢ/‖Mᵢ‖₂'
    elif r'\mathbf{x}_i' in compact and r'\sqrt{\alpha^2+\beta^2+\gamma^2}' in compact:
        ...
        rendered = 'd²_mix(i,j) = [α²‖K̂ᵢ − K̂ⱼ‖²₂ + β²‖P̂ᵢ − P̂ⱼ‖²₂ + γ²‖M̂ᵢ − M̂ⱼ‖²₂]/(α² + β² + γ²)'
    elif r'\beta^2\left(d_P^2-d_K^2\right)' in compact:
        rendered = 'β²(d²_P − d²_K) + γ²(d²_M − d²_K) < 0'
    else:
        rendered = re.sub(r'\\(left|right|Bigl|Bigr|quad|,|;)', '', compact)
...
def handle_placeholder(doc, line):
    if line.strip() == '[Insert FloatBarrier here.]':
        return True
    m = PLACEHOLDER_RE.match(line.strip())
    if not m:
        return False
    key = m.group(1).strip()
    # Supplementary assets are submitted in the separate supplementary PDF.
    if key.startswith('Supplementary Figure') or key.startswith('Supplementary Table'):
        return True
...
doc.add_paragraph('Ruixiang Mei and Jianhua Huang*', style=None).alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph('*Correspondence: Jianhua Huang, jhuang@cuhk.edu.cn', style=None).alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_paragraph('Ruixiang Mei ORCID: https://orcid.org/0009-0003-2128-0726', style=None).alignment = WD_ALIGN_PARAGRAPH.CENTER
