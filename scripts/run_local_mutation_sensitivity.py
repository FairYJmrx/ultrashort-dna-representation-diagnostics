...
def run_experiment(
    ...
    for (length, mode), subset in triplets.groupby(["source_length", "local_mode"], sort=True):
        ...
        for rep in representations:
            ...
            for row in evaluate_delta_readout(clean, noise, local, seed=seed, cv_folds=cv_folds):
                row.update(
                    {
                        "length": int(length),
                        "local_mode": mode,
...
def main() -> None:
    ...
    parser.add_argument(
        "--representations",
        default="ckmer4_count_l2,ckmer4_property_l2,ckmer4_property_multiscale_mean_l2,ckmer4_property_multiscale_l2,ckmer5_count_l2,property_channels,one_hot,base_property,rope_property,rope_onehot,kmer_property,cspaced_property_l2",
    )
