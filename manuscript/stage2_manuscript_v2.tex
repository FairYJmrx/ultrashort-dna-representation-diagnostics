...
\sloppy
\begin{document}
\title{\Large Controlled information-preservation diagnostics for ultra-short DNA read representations}

\author{\parbox{0.88\linewidth}{\centering\small Canonical k-mers preserve high-resolution identity evidence, whereas canonical spaced-property encoding provides compact perturbation-stable auxiliary evidence for mNGS-like reads}}

\date{\small MEI Ruixiang}
...
\section*{Terminology and Contribution}

We use one term consistently throughout the manuscript. \textbf{Canonical k-mer} denotes reverse-complement pooled contiguous k-mer counts. \textbf{Canonical spaced seed} denotes reverse-complement pooled counts of non-contiguous tokens. \textbf{CSP} denotes canonical spaced-property encoding, implemented in code as \texttt{cspaced\_property\_l2}. \textbf{Hybrid} denotes a concatenation of canonical k-mer counts and CSP followed by L2 normalization. \textbf{Perturbation stability} denotes similarity between a clean read and a mutated, N-masked, trimmed or locally mismatched version of the same read. It does not mean resistance to multi-species contamination.
...
\subsection*{Representation definitions}
...
\subsection*{Metrics and readout probes}
...
\subsection*{CSP had its clearest advantage in clean-perturbed stability}
...
\begin{quote}\small Full table omitted from the PDF body for readability (20 rows, 10 columns); see the Markdown manuscript and stage-2 table files.\end{quote}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_stability\_grid.png}
\caption{Clean-perturbed feature stability across read length.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_hospital\_69\_75\_l2.png}
\caption{Hospital-like 69/75 bp perturbation drift.}
\end{figure}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.153\linewidth}>{\raggedright\arraybackslash}p{0.153\linewidth}>{\raggedright\arraybackslash}p{0.153\linewidth}>{\raggedright\arraybackslash}p{0.153\linewidth}>{\raggedright\arraybackslash}p{0.153\linewidth}>{\raggedright\arraybackslash}p{0.153\linewidth}@{}}
\toprule
\textbf{Comparison} & \textbf{Mean cosine gain} & \textbf{Minimum gain} & \textbf{Maximum gain} & \textbf{Wins} & \textbf{Comparisons} \\
\midrule
...
\subsection*{Ablation showed that the property block, especially hydrogen-bond and entropy summaries, contributed to stability}
...
\toprule
\textbf{component} & \textbf{Mean\_cosine\_gain} & \textbf{Mean\_L2\_change} & \textbf{Max\_cosine\_gain} & \textbf{Min\_L2\_change} \\
\midrule
...
\toprule
\textbf{Condition} & \textbf{Length} & \textbf{Cosine gain over spaced seed} & \textbf{L2 change over spaced seed} & \textbf{mean cosine} & \textbf{L2 drift} & \textbf{Features} \\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_csp\_singleton\_ablation.png}
\caption{Singleton property ablation.}
\end{figure}

\subsection*{Lightweight readout probes separated robustness from identity resolution}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.184\linewidth}>{\raggedright\arraybackslash}p{0.184\linewidth}>{\raggedright\arraybackslash}p{0.184\linewidth}>{\raggedright\arraybackslash}p{0.184\linewidth}>{\raggedright\arraybackslash}p{0.184\linewidth}@{}}
\toprule
\textbf{Task} & \textbf{Representation} & \textbf{mean F1} & \textbf{accuracy} & \textbf{features} \\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_readout\_aggregate.png}
\caption{Lightweight readout probes remained task-dependent.}
\end{figure}

\subsection*{k and spaced-pattern sensitivity argued against a single-parameter recommendation}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}>{\raggedright\arraybackslash}p{0.131\linewidth}@{}}
\toprule
\textbf{Probe} & \textbf{Length} & \textbf{Method} & \textbf{Parameter} & \textbf{mean F1} & \textbf{sd F1} & \textbf{Mean features} \\
\midrule
target\_background & 69 & canonical spaced & pattern=0-2-5-7 & 0.555 & 0.085 & 136 \\
target\_background & 75 & k-mer & k=5 & 0.543 & 0.11 & 983.8 \\
target\_background & 150 & canonical k-mer & k=6 & 0.582 & 0.075 & 1957.2 \\
within\_genus\_species & 69 & canonical spaced + property & pattern=0-3-5-8 & 0.351 & 0.124 & 147 \\
within\_genus\_species & 75 & k-mer & k=5 & 0.373 & 0.135 & 979.5 \\
within\_genus\_species & 150 & canonical k-mer & k=6 & 0.398 & 0.181 & 1971.33 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
...
\subsection*{Deterministic neural probes showed model compatibility, not neural superiority}
...
\toprule
\textbf{Task} & \textbf{Model} & \textbf{Model family} & \textbf{Input} & \textbf{mean F1} & \textbf{Mean accuracy} & \textbf{Mean features} & \textbf{Runs} \\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_neural\_compatibility.png}
\caption{Deterministic neural compatibility probes.}
\end{figure}

\subsection*{Context loss around 125-150 bp was position-dependent, not a single read-length threshold}
...
\toprule
\textbf{Motif position} & \textbf{first full visibility} & \textbf{Maximum pair visibility} & \textbf{Lengths tested} \\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_attention\_breakpoints.png}
\caption{Attention-style context visibility breakpoints.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_attention\_f1\_breakpoints.png}
\caption{Best readout transitions around motif visibility.}
\end{figure}

\subsection*{ARG/SNP boundary probes bounded the role of CSP}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}>{\raggedright\arraybackslash}p{0.115\linewidth}@{}}
\toprule
\textbf{Boundary task} & \textbf{Condition} & \textbf{Length} & \textbf{Representation} & \textbf{mean cosine} & \textbf{L2 drift} & \textbf{retrieval} & \textbf{Features} \\
\midrule
arg\_allele & 3\% N mask & 69 & CSP & 0.994 & 0.105 & 0.125 & 78 \\
arg\_allele & 3\% N mask & 75 & CSP & 0.995 & 0.1 & 0.114 & 81 \\
arg\_allele & 6-bp mismatch & 69 & CSP & 0.99 & 0.139 & 0.119 & 78 \\
arg\_allele & 6-bp mismatch & 75 & CSP & 0.991 & 0.131 & 0.119 & 81 \\
arg\_family & 3\% N mask & 69 & CSP & 0.994 & 0.105 & 0.108 & 101 \\
arg\_family & 3\% N mask & 75 & CSP & 0.995 & 0.099 & 0.122 & 107 \\
arg\_family & 6-bp mismatch & 69 & CSP & 0.989 & 0.145 & 0.094 & 101 \\
arg\_family & 6-bp mismatch & 75 & CSP & 0.99 & 0.137 & 0.106 & 107 \\
resistance\_snp & 3\% N mask & 69 & CSP & 0.995 & 0.099 & 0.042 & 56 \\
resistance\_snp & 3\% N mask & 75 & CSP & 0.996 & 0.092 & 0.028 & 57 \\
resistance\_snp & 6-bp mismatch & 69 & CSP & 0.993 & 0.121 & 0.025 & 56 \\
resistance\_snp & 6-bp mismatch & 75 & CSP & 0.994 & 0.111 & 0.031 & 57 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
...
\textbf{task} & \textbf{Representation} & \textbf{mean F1} & \textbf{accuracy} & \textbf{features} \\
\midrule
arg\_allele & 5-mer+CSP & 1 & 1 & 407.417 \\
arg\_allele & 7-mer+CSP & 1 & 1 & 1160.88 \\
arg\_allele & CSP & 0.999 & 0.999 & 117.875 \\
arg\_allele & Canonical 5-mer & 0.999 & 0.999 & 289.542 \\
arg\_allele & Canonical 7-mer & 0.999 & 0.999 & 1043 \\
arg\_allele & c-spaced & 0.999 & 0.999 & 106.875 \\
arg\_family & CSP & 1 & 1 & 131 \\
arg\_family & Canonical 5-mer & 1 & 1 & 346.292 \\
arg\_family & 5-mer+CSP & 1 & 1 & 477.292 \\
arg\_family & Canonical 7-mer & 1 & 1 & 1264.5 \\
arg\_family & 7-mer+CSP & 1 & 1 & 1395.5 \\
arg\_family & c-spaced & 1 & 1 & 120 \\
resistance\_snp & 5-mer+CSP & 0.979 & 0.982 & 329.25 \\
resistance\_snp & CSP & 0.976 & 0.979 & 97.583 \\
resistance\_snp & c-spaced & 0.975 & 0.978 & 86.583 \\
resistance\_snp & Canonical 5-mer & 0.975 & 0.978 & 231.667 \\
resistance\_snp & 7-mer+CSP & 0.973 & 0.977 & 875.667 \\
resistance\_snp & Canonical 7-mer & 0.971 & 0.975 & 778.083 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/stage2\_fig\_arg\_snp\_readout.png}
\caption{ARG/SNP boundary readout probes.}
\end{figure}
