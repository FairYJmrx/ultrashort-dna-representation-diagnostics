...
Under the common historical-descriptor timing protocol, the optimized CK4P-MSP reference path required a median 2.71 s per 10,000 75-bp reads (interquartile range 1.51--2.86 s), compared with 2.15 s for CK4, 58.01 s for the current PseKNC implementation, 1.09 s for NCP+ANF and 0.89 s for PseEIIP. The separate 100,000-read pass required 28.72 s for CK4P-MSP and 21.73 s for CK4; the corresponding per-10,000-read times (2.87 and 2.17 s) were close to the 10,000-read medians. Runtime is implementation-specific, and the later PKM section reports a separate candidate-route extraction timing rather than a pooled estimate from this common descriptor wrapper. Taken together, the historical audit identifies a Pareto-like boundary: PseKNC is smaller and more invariant, whereas CK4P-MSP preserves a stronger, explicitly block-attributable response to structured local change (Supplementary Figure S12).
