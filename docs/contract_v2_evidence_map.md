...
| P/MSP relation and runtime | `experiments/audits/run_property_redundancy_and_runtime_audit.py` | `results/stage3/contract_v2/property_redundancy_runtime/` | CCA/rank summaries show relation rather than independence. Runtime uses each method's actual feature path; peak memory is a separate `tracemalloc` audit of Python-managed allocations. Both are implementation- and hardware-specific engineering boundaries. |
