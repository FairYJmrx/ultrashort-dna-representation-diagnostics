...
This checklist records the manuscript-level language constraints derived from the latest pre-submission reviewer-style audits. It should be used before rebuilding `paper_manuscript.docx` so that future context compaction does not lose the agreed boundaries.
...
- Keep Data Availability and Code Availability placeholders visible but mark them as submission blockers.
