...
## Latest read-length contract (2026-07-30)

- The paper is framed around controlled short reads, not a special 69-bp-only regime.
- Use the exact design phrase: `dense shared-template lower-bound sweep over every integer length from 50 to 75 bp, with broader anchors at 100, 125 and 150 bp`.
- The dense 50-75 bp sweep isolates length by prefix-truncating the same 420 source templates. It supports lower-range continuity, not a universal physical read-length law.
- The 100, 125 and 150 bp conditions are discrete anchors. Do not imply that 75-150 bp was sampled continuously or every 5 bp.
- The 69 and 75 bp values may be retained as aggregate local provenance markers, but no patient-level reads, labels, identifiers or sequence content entered the experiments.
- ART currently covers 50, 60, 69, 75, 100, 125 and 150 bp under the public block-normalized contract. Because the observed ART error load is non-monotonic across cycle profiles, interpret it only through within-length and within-quality-stratum comparisons.
- Supplementary Figure S13 is the authoritative lower-bound continuity figure; Supplementary Figure S3 is the authoritative current-contract ART error-load and quality-stratum figure.

## Locked positioning

- Paper type: methods-oriented representation diagnostics paper.
- Core claim: the paper diagnoses what information remains available in short-read representations under controlled perturbation, rather than introducing an end-to-end clinical classifier.
